...
\begin{figure}[h]
  \centerline{\fbox{\includegraphics[width=0.4\textwidth]{./Imagens/diodo_2_grafico.jpg}}}
...
O tempo de subida e descida $t_{sd}$ de uma porta digital indica a rapidez com que ela responde a uma mudança nas suas entradas. O tempo associado a uma mudança H $\rightarrow$ L na saída designa-se por $t_{pHL}$ e uma mudança de L $\rightarrow$ H é $t_{pLH}$. Então o $t_{sd}$ é a media:
...
\hline
$V_{rt}$ & $V_{1}$ & $V_{0} = \neg(\neg(V_{rt}*V_{1})*\neg(V{rt}*V{1}))$ \\ \hline
5 & 0 & 0 $\Rightarrow$ condensador carrega\\ \hline
5 & 5 & 1 $\Rightarrow$ condensador descarrega\\ \hline
\end{tabular}}
...
\begin{figure}[h]
  \centerline{\fbox{\includegraphics[width=0.4\textwidth]{./Imagens/parte3exercicio6.jpg}}}
  \caption{Vertical: 1V/div (ambos); \\Horizontal: 0,2ms/div (Time A)}\label{cmos}
\end{figure}
